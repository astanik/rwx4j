...
import de.tu_berlin.cit.rwx4j.rest.MethodType;
import de.tu_berlin.cit.rwx4j.rest.RestDocument;
import de.tu_berlin.cit.rwx4j.rest.MethodDocument.Method;
import de.tu_berlin.cit.rwx4j.rest.RequestDocument.Request;
...
public class MethodInvocation {
	...
	public MethodInvocation(RestDocument resourceDoc, de.tu_berlin.cit.rwx4j.xwadl.MethodDocument.Method method) {
		this.doc = resourceDoc;
		// set method type
		this.doc.getRest().addNewMethod().setType(MethodType.Enum.forString(method.getType().toString()));
		// set request
		if(method.isSetRequest()) {
			this.doc.getRest().getMethod().addNewRequest().setMediaType(method.getRequest().getMediaType());
		}
		// set response
		if(method.isSetResponse()) {
			this.doc.getRest().getMethod().addNewResponse().setMediaType(method.getResponse().getMediaType());
		}
	}
	
	public RestDocument getXmlDocument() {
		return this.doc;
	}

	public void setRequestRepresentation(String representation) {
		if (this.doc.getRest().getMethod().isSetRequest()) {
			Request request = this.doc.getRest().getMethod().getRequest();
